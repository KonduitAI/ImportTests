...
class ZooEvaluation(object):
    ...
    def write_intermediates(self, dest, force=True):
        ...
        if not (dest/"tf_model.pb").exists():
            copyfile(self.graphFile, dest/"tf_model.pb")
        ...
        with graph.as_default() as graph:
            ...
            with sess.as_default():
                ...
                to_save = {}
                for file, tensor in needed.items():
                    if len(to_save) >= EXEC_BACH:
                        try:
                            results = sess.run(to_save, feed_dict=feed_dict)
                ...
                for file, tensor in errors:
                    tensor: tf.Tensor
                    out = sess.run([tensor], feed_dict=feed_dict)[0]
                    print(tensor.name)
                    np.save(file, out)
...
if __name__ == '__main__':
    ...
    # z.graphFile("/TF_Graphs/ssd_mobilenet_v1_coco_2018_01_28/frozen_inference_graph.pb") \
    ...
    # z.graphFile("C:\\Users\\jimne\\Downloads\\cifar10_gan_85_frozenmodel.pb") \
    #     .inputName("input_1:0") \
